...
@app.route('/')
def index():
    face_classifier = cv2.CascadeClassifier(r'G:\eMO\MoodTunes\emotionDetection\haarcascade_frontalface_default.xml')
    classifier =load_model(r'G:\eMO\MoodTunes\emotionDetection\model.h5')
    ...
    fp.close()
    import os
    os.system(r'python G:\eMO\MoodTunes\songRecommender\test.py')
    print(max(zip(L.values(),L.keys()))[1])
    cap.release() # stop capturing frames
    cv2.destroyAllWindows()
    time.sleep(5)
    return "Hello World! still work in progress.."
